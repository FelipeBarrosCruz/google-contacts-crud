...
GoogleContactsParser.parseGetGroupsRawData = function (jsonData, callback) {
    var groups = lodash.get(jsonData, 'feed.entry')
    ...
    return callback(null, groups.map((group) => {
        return {
            shortmetadata: {
                id: lodash.get(group, 'id.$t'),
                etag: lodash.get(group, 'gd$etag'),
                updated: lodash.get(group, 'updated.$t'),
                category: lodash.get(group, 'category[0]'),
                title: lodash.get(group, 'title.$t'),
                content: lodash.get(group, 'content.$t'),
                link: lodash.get(group, 'link[0]'),
                systemGroup: lodash.get(group, 'gContact$systemGroup.id')
            },
            fullmetadata: group
        }
    }));
